window.KedrixOneDepartureNoticeModule = (() => {
  ...
  function buildDraftFromPractice(state, practice) {
    ...
    return createEmptyDraft(state, {
      ...
      atd: String(dynamic.atd || dynamic.eta || dynamic.etd || '').trim(),
  ...
  function fieldSize(name, options = {}) {
    ...
    const sm = new Set(['tripType', 'amount', 'documentDate', 'documentReceiptDate', 'emptyingAppointmentDate', 'operatorName', 'atd']);
  ...
  function renderSessionStrip(state, i18n) {
    ...
    return `
      <section class="panel practice-workspace-panel departure-notice-workspace-panel notice-workspace-panel">
        <div class="panel-head">
          <div>
            <h3 class="panel-title">${U.escapeHtml(i18n?.t('ui.openMasks', 'Maschere aperte'))}</h3>
            <p class="panel-subtitle">${U.escapeHtml(i18n?.t('ui.departureNoticeMasksHint', 'Più notifiche arrivo merce possono restare aperte contemporaneamente.'))}</p>
          </div>
          ...
          ${sessions.map((session) => {
            ...
            return `
              <div class="practice-session-chip notice-session-chip${active ? ' active' : ''}">
                <button class="notice-session-main" type="button" data-departure-notice-session-switch="${U.escapeHtml(session.id)}">
                  <strong>${U.escapeHtml(title)}</strong>
                  <span>${U.escapeHtml(meta || '—')}</span>
                  ${session.isDirty ? '<em>•</em>' : ''}
                </button>
                <button class="notice-session-close" type="button" data-departure-notice-session-close="${U.escapeHtml(session.id)}" aria-label="${U.escapeHtml(i18n?.t('ui.closeMask', 'Chiudi maschera'))}">×</button>
              </div>`;
          }).join('')}
        </div>
      </section>`;
  ...
  function renderLauncher(state, i18n, selectedPractice) {
    ...
    return `
      <section class="panel departure-notice-launcher-panel">
        <div class="panel-head">
          <div>
            <h3 class="panel-title">${U.escapeHtml(i18n?.t('ui.departureNoticeLauncherTitle', 'Apri o crea notifica partenza merce'))}</h3>
            <p class="panel-subtitle">${U.escapeHtml(i18n?.t('ui.departureNoticeLauncherHint', 'Apri il documento dalla pratica mare attiva o scegli una pratica per la partenza dall’elenco.'))}</p>
          </div>
        ...
        <div class="form-grid three departure-notice-practice-grid">
          ${available.slice(0, 18).map((practice) => `
            <button class="stack-item departure-notice-practice-chip" type="button" data-departure-notice-open-practice="${U.escapeHtml(practice.id)}">
  ...
  function renderSummaryPills(draft, i18n) {
    ...
    return `<div class="tag-grid departure-notice-summary-pills">${items.map(([label, value]) => `<div class="stack-item"><strong>${U.escapeHtml(label)}</strong><span>${U.escapeHtml(value)}</span></div>`).join('')}</div>`;
  }

  function renderLineTable(draft, i18n) {
    const rows = Array.isArray(draft.lineItems) ? draft.lineItems : [];
    return `
      <section class="table-panel departure-notice-lines-panel notice-lines-panel">
        <div class="panel-head">
          <div>
        ...
        <div class="table-wrap notice-lines-wrap">
          <table class="table departure-notice-lines-table notice-lines-table">
            <colgroup>
  ...
  function renderGeneralTab(draft, i18n) {
    const fields = [
      ...
      { label: i18n?.t('ui.departureNoticeTripType', 'Tipo viaggio'), name: 'tripType', value: draft.tripType, options: { type: 'select', items: [{ value: 'MARE', label: 'MARE' }, { value: 'AEREO', label: 'AEREO' }, { value: 'TERRA', label: 'TERRA' }] } },
      { label: i18n?.t('ui.sender', 'Mittente'), name: 'sender', value: draft.sender },
      { label: i18n?.t('ui.departureNoticeDestinationDepot', 'Deposito di partenza'), name: 'destinationDepot', value: draft.destinationDepot },
      ...
      { label: i18n?.t('ui.departureNoticeAttentionTo', 'All’attenzione di'), name: 'attentionTo', value: draft.attentionTo },
      { label: i18n?.t('ui.departureNoticeCompileLocation', 'Luogo compilazione'), name: 'compileLocation', value: draft.compileLocation },
      { label: i18n?.t('ui.date', 'Data'), name: 'documentDate', value: draft.documentDate, options: { type: 'date' } },
      { label: i18n?.t('ui.departureNoticeLoadingPort', 'Porto imbarco'), name: 'loadingPort', value: draft.loadingPort },
      { label: i18n?.t('ui.departureNoticeEtdEta', 'ATD'), name: 'atd', value: draft.atd },
      { label: i18n?.t('ui.departureNoticeUnloadingPort', 'Porto destino'), name: 'unloadingPort', value: draft.unloadingPort },
      { label: i18n?.t('ui.departureNoticeSupplierInvoice', 'Fattura fornitore'), name: 'supplierInvoice', value: draft.supplierInvoice },
      { label: i18n?.t('ui.amount', 'Importo'), name: 'amount', value: draft.amount },
      { label: i18n?.t('ui.departureNoticeGoodsType', 'Tipo merce'), name: 'goodsType', value: draft.goodsType },
      { label: i18n?.t('ui.voyage', 'Viaggio'), name: 'voyage', value: draft.voyage },
      { label: i18n?.t('ui.vessel', 'Nave'), name: 'vessel', value: draft.vessel },
      { label: i18n?.t('ui.departureNoticeDeliveryConditions', 'Condizioni di consegna'), name: 'deliveryConditions', value: draft.deliveryConditions },
      { label: i18n?.t('ui.departureNoticeOriginGoods', 'Origine merce'), name: 'originGoods', value: draft.originGoods },
      { label: i18n?.t('ui.customsSection', 'Sez. doganale'), name: 'customsSection', value: draft.customsSection },
      { label: i18n?.t('ui.departureNoticeOriginalNo', 'Original NO.'), name: 'originalNo', value: draft.originalNo },
      { label: i18n?.t('ui.departureNoticeOriginalCopies', 'Original copie'), name: 'originalCopyCount', value: draft.originalCopyCount },
      { label: i18n?.t('ui.operator', 'Operatore'), name: 'operatorName', value: draft.operatorName },
      { label: i18n?.t('ui.departureNoticeDocumentReceiptDate', 'Data invio documenti'), name: 'documentReceiptDate', value: draft.documentReceiptDate, options: { type: 'date' } },
      { label: i18n?.t('ui.departureNoticeEmptyingDate', 'Data partenza prevista'), name: 'emptyingAppointmentDate', value: draft.emptyingAppointmentDate, options: { type: 'date' } }
    ];
    return `
  ...
  function renderTextsTab(draft, i18n) {
    return `
      <div class="form-grid two departure-notice-form-grid">
        ${renderField(i18n?.t('ui.departureNoticeInternalText', 'Testo operativo'), 'internalText', draft.internalText, { type: 'textarea', rows: 8, full: true })}
  ...
  function buildPrintableHtml(draft, i18n) {
    const rows = Array.isArray(draft.lineItems) ? draft.lineItems : [];
    return `<section class="print-meta-grid"><div class="print-meta-card"><strong>${U.escapeHtml(i18n?.t('ui.generatedNumber', 'Pratica'))}</strong>${U.escapeHtml(draft.practiceReference || '—')}</div><div class="print-meta-card"><strong>${U.escapeHtml(i18n?.t('ui.clientRequired', 'Cliente'))}</strong>${U.escapeHtml(draft.client || '—')}</div><div class="print-meta-card"><strong>${U.escapeHtml(i18n?.t('ui.bookingWord', 'Booking'))}</strong>${U.escapeHtml(draft.bookingReference || '—')}</div><div class="print-meta-card"><strong>${U.escapeHtml(i18n?.t('ui.policyNumber', 'Polizza'))}</strong>${U.escapeHtml(draft.policyReference || '—')}</div></section><table><thead><tr><th>Container</th><th>Tipologia</th><th>Descrizione</th><th>Colli</th><th>Peso lordo</th><th>CBM</th></tr></thead><tbody>${rows.map((row)=>`<tr><td>${U.escapeHtml(row.containerCode || '')}</td><td>${U.escapeHtml(row.containerType || '')}</td><td>${U.escapeHtml(row.description || '')}</td><td>${U.escapeHtml(row.packageCount || '')}</td><td>${U.escapeHtml(row.grossWeight || '')}</td><td>${U.escapeHtml(row.cbm || '')}</td></tr>`).join('')}</tbody></table><section class="print-section"><h2>${U.escapeHtml(i18n?.t('ui.departureNoticeCustomerText', 'Testo destinatario'))}</h2><div class="print-block">${U.escapeHtml(draft.customerText || '').replace(/\n/g,'<br>')}</div></section>`;
  }
  ...
  function printDraft(draft, i18n, state) {
    if (DocumentOps && typeof DocumentOps.printHtmlDocument === 'function') {
      return DocumentOps.printHtmlDocument({
        ...
        clientConfig: DocumentOps?.resolveClientBranding ? DocumentOps.resolveClientBranding(state, draft) : { name: String(draft?.client || '').trim(), logoUrl: '' }
      });
    }
    return false;
  }
  ...
  function renderEditor(state, i18n) {
    ...
    return `
      <section class="panel departure-notice-editor-panel">
        <div class="panel-head">
          <div>
            <h3 class="panel-title">${U.escapeHtml(i18n?.t('practices/notifica-arrivo-merce', 'Notifica partenza merce'))}</h3>
        ...
        <div class="tab-row departure-notice-tabs">
  ...
  function renderSavedRecords(state, i18n) {
    ...
    return `
      <section class="panel">
        ...
        <div class="module-card-grid departure-notice-records-grid">
          ${records.length ? records.slice().sort((a, b) => String(b.updatedAt || '').localeCompare(String(a.updatedAt || ''))).map((record) => `
            <button class="module-card departure-notice-record-card" type="button" data-departure-notice-open-record="${U.escapeHtml(record.id)}">
  ...
  function bind(context = {}) {
    ...
    root.querySelectorAll('[data-departure-notice-session-close]').forEach((button) => {
      button.addEventListener('click', async (event) => {
        ...
        const closed = await closeSessionWithGuard(state, button.dataset.departureNoticeSessionClose, i18n);
        if (!closed) return;
        save?.();
        render?.();
      });
