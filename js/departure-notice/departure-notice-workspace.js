window.KedrixOneDepartureNoticeWorkspace = (() => {
  'use strict';
  ...
  function nextSessionId() {
    return `dn-session-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}`;
  }

  function defaultLineItem(overrides = {}) {
    return {
      id: `dn-line-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 7)}`,
      containerCode: '',
      containerType: '',
      description: '',
      seals: '',
  ...
  function ensureState(state, options = {}) {
    ...
    if (!state.departureNoticeWorkspace || typeof state.departureNoticeWorkspace !== 'object') {
      state.departureNoticeWorkspace = { activeSessionId: '', sessions: [] };
    }
    const workspace = state.departureNoticeWorkspace;
