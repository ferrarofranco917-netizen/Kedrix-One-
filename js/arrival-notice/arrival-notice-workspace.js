window.KedrixOneArrivalNoticeWorkspace = (() => {
  'use strict';
  ...
  function nextSessionId() {
    return `an-session-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}`;
  }

  function defaultLineItem(overrides = {}) {
    return {
      id: `an-line-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 7)}`,
      containerCode: '',
      containerType: '',
      description: '',
      seals: '',
  ...
  function ensureState(state, options = {}) {
    ...
    if (!state.arrivalNoticeWorkspace || typeof state.arrivalNoticeWorkspace !== 'object') {
      state.arrivalNoticeWorkspace = { activeSessionId: '', sessions: [] };
    }
    const workspace = state.arrivalNoticeWorkspace;
