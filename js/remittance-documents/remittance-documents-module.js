window.KedrixOneRemittanceDocumentsModule = (() => {
  ...
  function queueDispatchForDraft(state, record, i18n) {
    ...
    return DocumentOps.queueDispatch(state, {
      ...
      moduleLabel: i18n?.t('practices/rimessa-documenti', 'Rimessa documenti') || 'Rimessa documenti',
      documentLabel: i18n?.t('practices/rimessa-documenti', 'Rimessa documenti') || 'Rimessa documenti',
      ...
      subject: `${i18n?.t('practices/rimessa-documenti', 'Rimessa documenti') || 'Rimessa documenti'} ${String(record?.practiceReference || '').trim()}`.trim(),
      snapshot: {
  ...
  function buildPrintableHtml(draft, i18n) {
    const rows = Array.isArray(draft.lineItems) ? draft.lineItems : [];
    return `<section class="print-meta-grid"><div class="print-meta-card"><strong>${U.escapeHtml(i18n?.t('ui.generatedNumber', 'Pratica'))}</strong>${U.escapeHtml(draft.practiceReference || '—')}</div><div class="print-meta-card"><strong>${U.escapeHtml(i18n?.t('ui.clientRequired', 'Cliente'))}</strong>${U.escapeHtml(draft.client || '—')}</div><div class="print-meta-card"><strong>${U.escapeHtml(i18n?.t('ui.reference', 'Riferimento'))}</strong>${U.escapeHtml(draft.reference || '—')}</div><div class="print-meta-card"><strong>${U.escapeHtml(i18n?.t('ui.hawb', 'HAWB'))}</strong>${U.escapeHtml(draft.hawbReference || '—')}</div></section><table><thead><tr><th>${U.escapeHtml(i18n?.t('ui.documentType', 'Tipo documento'))}</th><th>${U.escapeHtml(i18n?.t('ui.reference', 'Riferimento'))}</th><th>${U.escapeHtml(i18n?.t('ui.copies', 'Copie'))}</th><th>${U.escapeHtml(i18n?.t('ui.notes', 'Note'))}</th></tr></thead><tbody>${rows.map((row)=>`<tr><td>${U.escapeHtml(row.documentType || '')}</td><td>${U.escapeHtml(row.reference || '')}</td><td>${U.escapeHtml(row.copies || '')}</td><td>${U.escapeHtml(row.notes || '')}</td></tr>`).join('')}</tbody></table><section class="print-section"><h2>${U.escapeHtml(i18n?.t('ui.remittanceDocumentsCustomerText', 'Testo cliente'))}</h2><div class="print-block">${U.escapeHtml(draft.customerText || '').replace(/\n/g,'<br>')}</div></section>`;
  }
      ...
      <section class="panel remittance-documents-editor-panel">
        <div class="panel-head">
          ...
          <div class="action-row">
            ...
            <button class="btn secondary" type="button" data-remittance-email>${U.escapeHtml(i18n?.t('ui.saveAndSend', 'Salva e invia'))}</button>
